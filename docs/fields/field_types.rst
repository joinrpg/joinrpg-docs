...
.. attention:: Если эти кнопки вам не видны, а добавление новых полей недоступно — у вас нет `права настраивать поля для проекта </project/access.html#id3>`_.

**Тип поля** — что это за поле. Нельзя менять после создания поля. 

**Привязано к персонажу / заявке**. `Подробнее про разницу между персонажем и заявкой </fields/character_vs_application.html>`_. Нельзя менять после создания поля.
...
Как обязательные, так и рекомендованные поля используются при проверке, готова ли заявка к `прохождению регистрации </checkin/before.html>`_.
...
**Показывать только для групп** — поле будет показываться только для `определенной группы </groups/index.html>`_ (например, список заклинаний — только для магов). Есть два сценария использования:
